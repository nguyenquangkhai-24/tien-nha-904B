  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={onClose}>
      <div className="relative max-w-md w-full flex flex-col items-center" onClick={e => e.stopPropagation()}>
        ...
        <div 
          ref={receiptRef}
          className="w-full bg-slate-900 rounded-2xl overflow-hidden border border-slate-700 shadow-2xl relative"
        >
          {/* Header */}
          <div className="bg-gradient-to-br from-emerald-500 to-teal-600 p-6 text-center relative overflow-hidden">
            ...
            <Building2 className="w-10 h-10 mx-auto text-white mb-2 opacity-90" />
            <h2 className="text-2xl font-black text-white tracking-wide uppercase shadow-sm">Phòng 904B</h2>
            <p className="text-emerald-50 font-medium mt-1">Hoá Đơn Tiền Nhà - Tháng {month}/{year}</p>
          </div>

          {/* Body */}
          <div className="p-6 space-y-5 bg-slate-900">
            <div className="flex justify-between items-center pb-4 border-b border-slate-700 border-dashed">
              <span className="text-slate-400 text-sm">Người thanh toán:</span>
              <span className="text-lg font-bold text-slate-100">{memberData.name}</span>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center text-sm">
                <span className="text-slate-400">Đồ dùng phát sinh (chia 6)</span>
          ...
          <div className="bg-slate-800 p-6 border-t border-slate-700">
            <div className="flex justify-between items-end">
              <div className="flex items-center gap-1.5 text-emerald-400">
                ...
                <span className="font-bold uppercase tracking-wider text-sm">Tổng Thanh Toán</span>
              </div>
              <span className="text-2xl font-black text-emerald-400 font-mono">{formatVND(memberData.total_due)}</span>
            </div>
